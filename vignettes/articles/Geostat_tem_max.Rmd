...
hlp_install("climaemet") # metereological data
hlp_install("mapSpain") # base maps of Spain
hlp_install("raster") # raster handling
hlp_install("sf") # spatial shape handling
hlp_install("gstat") # for spatial interpolation
hlp_install("dplyr") # data handling
hlp_install("ggplot2") # for plots
hlp_install("gifski") # we would create an animation
hlp_install("geoR") # we would create an animation
hlp_install("classInt") # we would create an animation
```
...
applications, so there is also a myriad of spatial data types.
[@doi:https://doi.org/10.1002/9781119115151.ch1] GEMA COMPROBAR REFERENCIAS
provides a simple and useful classification of spatial data: **geostatistical
data** (air temperature values in a country), **lattice data** (unemployment
rate by states) and **point patterns** (the location of fires in a region), see
[@montero_spatial], for more details.
...
are going to model the level of air temperature in Spain in XXXXX (fecha)

DIEGO?? We are going to download the data with with climaemet package in R which
has an API ...?
...
DIEGO SUPRIMIMOS LAS DOS O DEJAMOS BALEARES O DEJAMOS TAMBIEN CANARIAS, TOTAL
COMO LUEGO VAMOS A RECORTAR NO VEO TANTO PROBLEMA.

Note: For a simple of the compresion we delete the Spanis Island (Canary and
Balearic)
...
Second, we examine the possible variables that can be analyzed. We are
interested in **maximum temperature** (hour frequency).
...
Third, we download and plot the maximum temperature (hour frequency) for each
monitoring station around the county.
...
distant things ([@https://doi.org/10.1111/j.1538-4632.1969.tb00621.x].

In our study, we can observe positive spatial dependence (see Figure XX), high
...
In this work we deal with rdinary kriging, the most widely used kriging method.
